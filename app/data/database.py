...
class TokenUsage(db.Model):
    ...
    model_name = db.Column(db.String(100), default="Gemma 3 4B (Local)")
...
def _migrate_token_usage_model_name():
    """Add model_name column to token_usage if it doesn't exist (non-Alembic migration)."""
    from sqlalchemy import text, inspect as sa_inspect
    inspector = sa_inspect(db.engine)
    columns = [col["name"] for col in inspector.get_columns("token_usage")]
    if "model_name" not in columns:
        db.session.execute(
            text('ALTER TABLE token_usage ADD COLUMN model_name VARCHAR(100) DEFAULT "Gemma 3 4B (Local)"')
        )
        db.session.commit()
...
def init_db(app):
    ...
    with app.app_context():
        db.create_all()
        _migrate_token_usage_model_name()
